...
wss.on('connection', function (ws) {
    ...
    ws.on('message', async function (message) {
        ...
        switch (body.type) {
            case 'join-meeting':
                const meetingIds = [];
                    for (let peer of peers) {
                        Object.values(peer).map(meeting => {
                            meeting.meetingId && meetingIds.push(meeting.meetingId);
                        });
                    }
                    const isMeetingExist = meetingIds.some(item => item === body.meetingId);
                    console.log(body.meetingId, isMeetingExist, meetingIds);
            ...
            break;
            case 'connect':
                const meetingId = body.meetingId || uuidv4();
                peers.set(body.uqid, { socket: ws });
                const peer = createPeer();
                peers.get(body.uqid).username = body.username;
                peers.get(body.uqid).meetingId = meetingId;
